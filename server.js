...
function getNetworkEvents() {
  ...
  // Statische Fallback-Events für Hamburg
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/sources') {
    const sources = [
      { name: 'Eigene Datenbank', icon: '🗄️', description: 'Kuratierte Locations aus OSM, Web-Recherche — vollständig kontrolliert', priority: 1, status: 'ok', statusText: 'Aktiv', count: HIGHLIGHTS.length },
...
server.listen(PORT, '127.0.0.1', () => console.log(`Barfinder running on port ${PORT}`));
